...
def topology(pathmanager, scheduler):
    ...
    cpu = .00001
    if sched == 'cfs':
        cpu *= .5  # for cfs division by 2
    period = 100000
    "Create a network."
    net = Mininet(controller=RemoteController,
                  link=TCLink, switch=OVSKernelSwitch, host=CPULimitedHost)
    net.start()
    print "*** Creating nodes"
    sta1 = net.addStation(
        'sta1', wlans=2, ip='10.0.0.10/8', position='51,10,0')
    ap2 = net.addAccessPoint('ap2', mac='00:00:00:00:00:02', equipmentModel='TLWR740N',
                             protocols='OpenFlow10', ssid='ssid_ap2', mode='n', channel='11', position='55,17,0')
    ap3 = net.addAccessPoint('ap3', mac='00:00:00:00:00:03', equipmentModel='TLWR740N',
                             protocols='OpenFlow10', ssid='ssid_ap3', mode='n', channel='1', position='50,11,0')
    # h4 = net.addHost('h4', mac='00:00:00:00:00:04',
    #                  ip='10.0.0.254/8')
    h4 = net.addHost('h4', mac='00:00:00:00:00:04',
                     ip='10.0.0.254/8', cpu=cpu, sched=sched, period_us=period)
    ...
    #                   ip='192.168.1.254/24', cpu=cpu, sched=sched, period_us=period)
    c11 = net.addController('c11', controller=RemoteController, ip='127.0.0.1')
    # printing CPU usage for configured host
    if sched == 'rt':
        print h4.rtInfo(cpu)
        print "*** Percentage of CPU usage: %f%% ***" % (cpu * 100)
    elif sched == 'cfs':
        print h4.cfsInfo(cpu)
        print "*** Percentage of CPU usage: %f%% ***" % (cpu * 100 * 2)
    ...
    net.addLink(ap2, h4, bw=10, use_htb=True, max_queue_size=1000)
    ...
    h10.cmd('iperf -s -i 1 > testcpu/testcmd/server/server_' +
            name_postfix + '.log &')
    sta1.cmd('iperf -c 192.168.1.254 -t 30 -i 1 > testcpu/testcmd/client/iperf/client_' +
             name_postfix + '.log 2>&1 &')
    sta1.cmd('ifstat -b > testcpu/testcmd/client/ifstat/ifstat_' +
             name_postfix + '.log 2>&1 &')
    ...
    sta1.cmd('cat ' + proc_path + ' > testcpu/testcmd/client/proc/proc_' +
             name_postfix + '.log')
