...
def topology(pathmanager, scheduler):
    #  mobility distance: 63.25 m in time: 100s, velocity: 0.63 m/s
    #  distances at start:
    #  ap2 - ap3: 10.30m
    #  sta1 - ap2: 26.92m
    #  sta1 - ap3: 20.02m
    #  distances at the end of mobility:
    #  sta1 - ap2: 36.40
    #  sta1 - ap3: 44.28
    FOLDER_NAME = "refactored"
    "Create a network."
    ...
    os.system('rm testmobility/' + FOLDER_NAME +
    ...
    h10.cmd('iperf -s -i 1 >> testmobility/' + FOLDER_NAME +
            '/server/server_' + name_postfix + '.log &')
    sta1.cmd('iperf -c 192.168.1.254 -t 120 -i 1 >> testmobility/' + FOLDER_NAME + '/client/iperf/client_' +
             name_postfix + '.log 2>&1 &')
    sta1.cmd('ifstat -b >> testmobility/' + FOLDER_NAME + '/client/ifstat/ifstat_' +
             name_postfix + '.log 2>&1 &')
    ...
    sta1.cmd('cat ' + proc_path + ' >> testmobility/' + FOLDER_NAME + '/client/proc/proc_' +
             name_postfix + '.log')

    # lets wait till it is finshed
    sleep(17)
    sta1.cmd('echo \"*** starting mobility***\" >> testmobility/' + FOLDER_NAME + '/client/iperf/client_' +
             name_postfix + '.log ')
    sta1.cmd('echo \"*** starting mobility***\" >> testmobility/' + FOLDER_NAME + '/client/ifstat/ifstat_' +
             name_postfix + '.log ')
    print "starting mobility"
    sleep(100)
    sta1.cmd('echo \"*** stopping mobility***\" >> testmobility/' + FOLDER_NAME + '/client/iperf/client_' +
             name_postfix + '.log ')
    sta1.cmd('echo \"*** stopping mobility***\" >> testmobility/' + FOLDER_NAME + '/client/ifstat/ifstat_' +
             name_postfix + '.log ')
    print "stopping mobility"
    sleep(9)
