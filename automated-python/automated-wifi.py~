...
def topology(pathmanager, scheduler):
    ...
    h10.cmd('iperf -s -i 1 > server/server_' + name_postfix + '.log &')
    sta1.cmd('iperf -c 192.168.1.254 -t 30 -i 1 > client/iperf/client_' +
             name_postfix + '.log 2>&1 &')
    sta1.cmd('ifstat -b > client/ifstat/ifstat_' +
             name_postfix + '.log 2>&1 &')
    ...
    sta1.cmd('cat ' + proc_path + ' > client/proc/proc_' +
             name_postfix + '.log')
