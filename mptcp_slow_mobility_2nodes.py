...
def topology():
    ...
    sta1 = net.addStation(
        'sta1', wlans=2, ip='10.0.0.10/8', position='52,10,0')
    sta2 = net.addStation(
        'sta2', wlans=2, ip='10.0.0.20/8', position='48,10,0')
    ap2 = net.addAccessPoint('ap2', mac='00:00:00:00:00:02', equipmentModel='TLWR740N',
                             protocols='OpenFlow10', ssid='ssid_ap2', mode='n', channel='6', position='46,12,0')
    ap3 = net.addAccessPoint('ap3', mac='00:00:00:00:00:03', equipmentModel='TLWR740N',
                             protocols='OpenFlow10', ssid='ssid_ap3', mode='n', channel='1', position='56,20,0')
    ...
    #  sta2 config
    sta2.cmd('ifconfig sta2-wlan0 10.0.0.20/8')
    sta2.cmd('ifconfig sta2-wlan1 192.168.0.20/24')

    sta2.cmd('ip route add default 10.0.0.254/8 via sta2-wlan0')
    sta2.cmd('ip route add default 192.168.0.254/24 via sta2-wlan1')

    sta2.cmd('ip rule add from 10.0.0.20 table 1')
    sta2.cmd('ip rule add from 192.168.0.20 table 2')

    sta2.cmd('ip route add 10.0.0.0 dev sta2-wlan0 scope link table 1')
    sta2.cmd('ip route add default via 10.0.0.254 dev sta2-wlan0 table 1')

    sta2.cmd('ip route add 192.168.0.0 dev sta2-wlan1 scope link table 2')
    sta2.cmd('ip route add default via 192.168.0.254 dev sta2-wlan1 table 2')
    ...
    # uncoment this to plot graph and start mobility of sta1
    # print"*** Plot graph ***"
    # net.plotGraph(max_x=100, max_y=100)

    # net.startMobility(time=0)
    # net.mobility(sta1, 'start', time=30, position='40.0,30.0,0.0')
    # net.mobility(sta1, 'stop', time=90, position='80.0,10.0,0.0')
    # net.stopMobility(time=90)
